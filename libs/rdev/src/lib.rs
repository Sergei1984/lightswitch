...
#[cfg(target_os = "macos")]
use crate::macos::{display_size as _display_size, listen as _listen, listen_main as _listen_main, simulate as _simulate};
...
pub fn listen_main<T>(callback: T) -> Result<(), ListenError>
where
    T: FnMut(Event) + 'static,
{
    _listen_main(callback)
}
